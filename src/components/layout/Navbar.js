  ...
  return (
    <nav className={`sticky top-0 z-40 bg-white transition-all ${isScrolled ? 'border-b border-[rgba(0,0,0,0.1)] shadow-sm' : ''}`}>
      <div className="max-w-[1240px] w-full mx-auto px-4 md:px-6 lg:px-4 xl:px-6 h-16 md:h-24 flex items-center justify-between">
        ...
        {/* Logo */}
        <Link href="/" className="font-integral text-[24px] md:text-[32px] font-bold text-black tracking-tight ml-3 lg:ml-0 lg:mr-2 xl:mr-6 flex-shrink-0 flex items-center justify-center">
          SHOP.CO
        </Link>
          ...
          >
            ...
            {isShopDropdownOpen && (
              <div className="absolute top-full left-0 bg-white shadow-md border border-[rgba(0,0,0,0.1)] rounded-lg py-2 min-w-[160px] z-50">
                <Link href="/category/t-shirts" className="block px-4 py-2 hover:bg-[#F0F0F0] transition-colors">T-shirts</Link>
                <Link href="/category/jeans" className="block px-4 py-2 hover:bg-[#F0F0F0] transition-colors">Jeans</Link>
                <Link href="/category/shirts" className="block px-4 py-2 hover:bg-[#F0F0F0] transition-colors">Shirts</Link>
                <Link href="/category/shorts" className="block px-4 py-2 hover:bg-[#F0F0F0] transition-colors">Shorts</Link>
              </div>
            )}
          </div>
          <Link href="/category/casual" className="hover:text-gray-600 transition-colors">On Sale</Link>
          <Link href="/category/casual" className="hover:text-gray-600 transition-colors">New Arrivals</Link>
          <Link href="/category/casual" className="hover:text-gray-600 transition-colors">Brands</Link>
        </div>

        {/* Search Bar (Desktop) */}
        <form onSubmit={submitSearch} className="hidden lg:flex flex-1 max-w-[480px] lg:max-w-[260px] xl:max-w-[480px] lg:mx-2 xl:mx-5 relative min-w-[100px]">
          ...
          <input 
            ...
            value={searchQuery}
            onChange={(event) => { setSearchQuery(event.target.value); setIsSearchResultsOpen(true); }}
            onFocus={() => searchQuery.trim().length >= 2 && setIsSearchResultsOpen(true)}
          ...
          />
          {searchQuery && <button type="button" onClick={clearSearch} className="absolute right-3 top-1/2 -translate-y-1/2 z-20 w-7 h-7 rounded-full bg-black text-white flex items-center justify-center hover:bg-gray-700 transition-colors" aria-label="Clear search"><IoClose size={17}/></button>}
          {searchQuery.trim().length >= 2 && isSearchResultsOpen && <div className="absolute left-0 right-0 top-[54px] rounded-2xl bg-white border border-black/10 shadow-xl overflow-hidden z-[70]">
        ...
        <div className="flex items-center space-x-3 md:space-x-5 flex-shrink-0 ml-auto lg:ml-0">
          <button onClick={() => setIsMobileSearchOpen((value) => !value)} className="lg:hidden text-black hover:text-gray-600 transition-colors" aria-label="Search">
            <IoSearchOutline size={24} className="md:w-7 md:h-7" />
          </button>
          
          <Link href="/cart" className="relative text-black hover:text-gray-600 transition-colors" aria-label="Cart">
          ...
          {user && <div className="relative"><button onClick={openNotifications} className="relative text-black hover:text-gray-600" aria-label={language === "ru" ? "Уведомления" : "Notifications"}><IoNotificationsOutline size={24} className="md:h-7 md:w-7"/>{unreadCount > 0 && <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#FF3333] px-1 text-[10px] font-bold text-white">{unreadCount}</span>}</button>{notificationsOpen && <div className="absolute right-0 top-10 z-[80] w-[min(340px,calc(100vw-32px))] overflow-hidden rounded-2xl border border-black/10 bg-white shadow-2xl"><div className="border-b border-black/10 px-4 py-3 font-bold">{language === "ru" ? "Уведомления" : "Notifications"}</div>{notifications.length ? <div className="max-h-80 overflow-y-auto">{notifications.map((item)=><Link key={item.id} href="/account" onClick={()=>setNotificationsOpen(false)} className="block border-b border-black/5 px-4 py-3 last:border-0 hover:bg-[#f2f2f2]"><p className="text-sm">{formatNotificationMessage(item.message, language)}</p><span className="mt-1 block text-xs text-black/40">{new Date(item.created_at).toLocaleString(language === "ru" ? "ru-RU" : "en-US")}</span></Link>)}</div> : <p className="px-4 py-5 text-sm text-black/50">{language === "ru" ? "Уведомлений пока нет." : "No notifications yet."}</p>}</div>}</div>}
      ...
      {isMobileSearchOpen && <form onSubmit={submitSearch} className="lg:hidden px-4 pb-4 relative bg-white">
      ...
      <div className={`fixed top-0 left-0 h-full w-[280px] bg-white z-50 transform transition-transform duration-300 ease-in-out lg:hidden shadow-xl flex flex-col ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 flex items-center justify-between border-b border-[rgba(0,0,0,0.1)]">
          <span className="font-integral text-[24px] font-bold text-black tracking-tight">SHOP.CO</span>
          <button onClick={() => setIsMenuOpen(false)} aria-label="Close Menu" className="text-black hover:text-gray-600 transition-colors">
            <IoClose size={28} />
          </button>
        </div>
        <div className="p-4 flex flex-col space-y-6 font-satoshi text-lg mt-4">
          <Link href="/category/casual" className="font-medium hover:text-gray-600 transition-colors" onClick={() => setIsMenuOpen(false)}>Shop</Link>
          <Link href="/category/casual" className="font-medium hover:text-gray-600 transition-colors" onClick={() => setIsMenuOpen(false)}>On Sale</Link>
          <Link href="/category/casual" className="font-medium hover:text-gray-600 transition-colors" onClick={() => setIsMenuOpen(false)}>New Arrivals</Link>
          <Link href="/category/casual" className="font-medium hover:text-gray-600 transition-colors" onClick={() => setIsMenuOpen(false)}>Brands</Link>
        </div>
      </div>
    </nav>
  );
